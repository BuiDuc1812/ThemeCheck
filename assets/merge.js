...
parentNodeUl.forEach(item=>{
    ...
    const createLi = function(){
        ...
        var first = item.getAttribute('first');
        var second = item.getAttribute('second');
        var third = item.getAttribute('third');
        var fouth = item.getAttribute('fouth');
        ...
        arr.forEach(item=>{
            ...
            item.insertBefore(newLi,liSale);
        })
    }
    createLi();
    console.log(item)
    function addItem(){
        ...
        console.log(newCols)
        var list = item.querySelectorAll('.merge');
